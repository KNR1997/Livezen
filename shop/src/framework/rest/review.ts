...
export function useReviews_v2(options?: Partial<ReviewQueryOptions>) {
  const {
    data: response,
    isLoading,
    error,
    ...
    [API_ENDPOINTS.PRODUCTS_REVIEWS_v2, options],
    ({ queryKey }) =>
      client.reviews_v2.all(Object.assign({}, queryKey[1] as ReviewQueryOptions)),
    {
      keepPreviousData: true,
    },
  );
  return {
    reviews_v2: response?.data ?? [],
    paginatorInfo: mapPaginatorData(response),
    isLoading,
    error,
    isFetching,
