  ...
  const {
      ...
      client.products.all(Object.assign({}, queryKey[1], pageParam)),
    {
      getNextPageParam: ({ current_page, last_page }) => {
        return last_page > current_page ? { page: current_page + 1 } : undefined;
      },
    },
  );
...
// Django method
export function useProducts_v2(options?: Partial<ProductQueryOptions>) {
  const { locale } = useRouter();
  ...
  const {
    ...
    [API_ENDPOINTS.PRODUCTS_v2, formattedOptions],
    ({ queryKey, pageParam }) =>
      client.products_v2.all(Object.assign({}, queryKey[1], pageParam)),
    {
      getNextPageParam: ({ current_page, last_page }) =>
  ...
  return {
    products_v2: data?.pages?.flatMap((page) => page.data) ?? [],
    paginatorInfo: Array.isArray(data?.pages)
      ? mapPaginatorData(data?.pages[data.pages.length - 1])
      : null,
    isLoading,
...
export const usePopularProducts_v2 = (
  options?: Partial<PopularProductQueryOptions>,
) => {
  const { locale } = useRouter();
  ...
  const { data, isLoading, error } = useQuery<Product[], Error>(
    [API_ENDPOINTS.PRODUCTS_POPULAR_v2, formattedOptions],
    ({ queryKey }) =>
      client.products_v2.popular(queryKey[1] as PopularProductQueryOptions),
  );
...
export function useProduct_v2({ slug }: { slug: string }) {
  const { locale: language } = useRouter();

  const { data, isLoading, error } = useQuery<Product, Error>(
    [API_ENDPOINTS.PRODUCTS_v2, { slug, language }],
    () => client.products_v2.get({ slug, language }),
  );
  return {
    product_v2: data,
    isLoading_v2: isLoading,
    error,
  };
}
...
export function useQuestions_v2(options?: Partial<QuestionQueryOptions>) {
  const {
    data: response,
    isLoading,
    error,
    ...
    [API_ENDPOINTS.PRODUCTS_QUESTIONS_v2, options],
    ({ queryKey }) =>
      client.products_v2.questions(
        Object.assign({}, queryKey[1] as QuestionQueryOptions),
      ),
    {
      keepPreviousData: true,
